...
--
--  NOTES:
--      Creates rounded rectanges. Nothing needs to be changed here besides size
--      values for different resolution.
----------------------------------------------------------------------------*/
...
RoundRectItem::RoundRectItem(const QRectF &rect, const QBrush &brush, QWidget *embeddedWidget) : QGraphicsRectItem(rect), brush(brush), timeLine(75), lastVal(0), opa(1), proxyWidget(0)
{
    connect(&timeLine, SIGNAL(valueChanged(qreal)), this, SLOT(updateValue(qreal)));
...
void RoundRectItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *qw)
{
    ...
    {
        proxyWidget->setGeometry(boundingRect().adjusted(15, 15, -5, -5));
    }
...
{
    if(event->isAutoRepeat() || event->key() != Qt::Key_Return || (timeLine.state() == QTimeLine::Running && timeLine.direction() == QTimeLine::Forward))
    {
